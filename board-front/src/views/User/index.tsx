...
export default function User() {
  ...
  const UserTop = () => {
    ...
    //     function: patch profile image response처리 함수
    ...
    //     function: patch nickname response처리 함수
    ...
    //     useEffect: email path variable 변경시 실행할 함수
    useEffect(() => {
      if (!userEmail) return;
      getUserRequest(userEmail).then(getUserResponse);
    }, [userEmail]);
    ...
    return(
      <div id='user-bottom-wrapper'>
        <div className='user-bottom-container'>
          <div className='user-bottmom-title'>{isMyPage ? '내 게시물 ' : '게시물 '}<span className='emphasis'>{count}</span></div>
          <div className='user-bottmom-contents-box'>
            {count === 0 ?
          ...
          <div className='user-bottmom-pagination-box'>
            {count !==0 &&
            <Pagination 
