...
export default function Authentication() {
  ...
  const SignInCard = () => {
    ...
    //  status: 이메일,패스워드 요소 참조 상태
    const emailRef = useRef<HTMLInputElement | null>(null);
    const passwordRef = useRef<HTMLInputElement | null>(null);

    //  status: 이메일, 패스워드, 패스워드 타입, 에러 상태
    const [email, setEmail] = useState<string>('');
    const [password, setPassword] = useState<string>('');
    const [passwordType, setPasswordType] = useState<'text' | 'password'>('password');
    const [error, setError] = useState<boolean>(false);

    //  status: 패스워드 버튼 아이콘 상태
    ...
    //  event handler: 이메일, 패스워드드 인풋 키 다운 이벤트 처리
  ...
  const SignUpCard = () => {
    ...
    //   state: 닉네임, 전화번호, 주소, 상세주소 요소 참조 상태
    ...
    //   state: 이메일, 패스워드, 패스워드 확인 에러 상태
    const [isEmailError, setEmailError] = useState<boolean>(false);
    const [isPasswordError, setPasswordError] = useState<boolean>(false);
    const [isPasswordCheckError, setPasswordCheckError] = useState<boolean>(false);

    //   state: 닉네임, 전화번호, 주소, 상세주소 에러 상태
    ...
    //   state: 이메일, 패스워드, 패스워드 확인 에러 메세지 상태
    const[emailErrorMessage, setEmailErrorMessage] = useState<string>('');
    const[passwordErrorMessage, setPasswordErrorMessage] = useState<string>('');
    const[passwordCheckErrorMessage, setPasswordCheckErrorMessage] = useState<string>('');

    //   state: 닉네임, 전화번호, 주소, 상세주소 에러 메세지 상태
    const [nicknameErrorMessage, setNicknameErrorMessage] = useState<string>('');
    const [telNumberErrorMessage, setTelNumberErrorMessage] = useState<string>('');
    const [addressErrorMessage, setAddressErrorMessage] = useState<string>('');

    //   state: 패스워드, 패스워드 확인 버튼 아이콘 상태
    const [passwordButtonIcon, setPasswordButtonIcon] = useState<'eye-light-off-icon' | 'eye-light-on-icon'>('eye-light-off-icon');
    ...
    //   event handler: 이메일, 패스워드, 패스워드 확인 변경 이벤트 처리
    const onEmailChangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
      const { value } = event.target;
      setEmail(value);
      setEmailError(false);
    ...
    //    event handler: 닉네임, 전화번호, 주소, 상세주소 변경 이벤트 처리
    ...
    //    event handler: 패스워드 버튼, 패스워드 확인 버튼 클릭 이벤트 처리
    const onPasswordButtonClickHandler = () => {
    ...
    //   event handler: 이메일, 패스워드, 패스워드 확인 키 다운 이벤트 처리
    const onEmailKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
      if (event.key !== 'Enter') return;
      if (!passwordRef.current) return;
      passwordRef.current.focus();
    ...
    //   event handler: 닉네임, 전화번호, 주소, 상세주소 키 다운 이벤트 처리
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          ...
          <div className='auth-card-bottom'>
            ...
            <div className='auth-description-box'></div>
        ...
        <div className='auth-jumbotron-box'>
          <div className='auth-jumbotron-contents'>
            ...
            <div className='auth-jumbotron-text-box'>
              ...
              <div className='auth-jumbotron-text'>{'게시판 입니다.'}</div>
            </div>
          </div>
        </div>
